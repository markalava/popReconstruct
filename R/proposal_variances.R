...
conditional.variances <- function(results.recon, plot = TRUE, return.res = TRUE) {
    ...
    vitalCondVars$fert.rate <-
        chain.cond.vars(log(results.recon$fert.rate.mcmc))

    vitalCondVars$surv.prop <-
        (chain.cond.vars(logit(results.recon$surv.prop.mcmc[["female"]])) +
         chain.cond.vars(logit(results.recon$surv.prop.mcmc[["male"]]))) / 2

    vitalCondVars$mig <-
        (chain.cond.vars(results.recon$mig.prop.mcmc[["female"]]) +
         chain.cond.vars(results.recon$mig.prop.mcmc[["male"]])) / 2

    vitalCondVars$population.count <-
        (chain.cond.vars(log(results.recon$baseline.count.mcmc[["female"]])) +
         chain.cond.vars(log(results.recon$baseline.count.mcmc[["male"]]))) / 2

    vitalCondVars$srb <-
        chain.cond.vars(log(results.recon$srb.mcmc))

    vitalCondVars$variances <-
        chain.cond.vars(log(results.recon$variances.mcmc))
