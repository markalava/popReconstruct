             ...
             )
{
    ...
    ##-- The log posterior is the SUM of log.prior.two.sex and log.like --##

    return(sum(log.prior.two.sex(f, s, g, baseline.n, srb
...
popRecon.sampler.two.sex <- function(n.iter, burn.in = 0,
    ...
    log.curr.proj <-
        lapply(proj.cen.yrs.two.sex(full.proj =
        ...
        for(j in 1:length(log.curr.f[fert.rows,])) {
            ...
            if(sum(full.proj.unlist < 0) > 0 || is.na(sum(full.proj.unlist)) ||
               is.nan(sum(full.proj.unlist))) {
                ...
                prop.proj <-
                    proj.cen.yrs.two.sex(full.proj = full.proj
                                                ,bline.yr = baseline.year
                                                ,vr.yrs = vr.years
                                                ,cen.yrs = census.years, proj.yrs = proj.years
                                                ,labels = FALSE
                ...
                log.prop.posterior <-
                    log.post.two.sex(f = log.prop.f #<-- use proposal
                                            ,s = logit.curr.s
                                            ,g = curr.g
                                            ,baseline.n = log.curr.b
                                            ,srb = log.curr.srb
